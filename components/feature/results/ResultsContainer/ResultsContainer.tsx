...
const Results: React.FC<ResultsProps> = ({ profile, answers, questions, onRestart, isAuthenticated = true, onRegisterRequired }) => {
  const { t, i18n } = useTranslation();
  const language = i18n.language as 'en' | 'id';
  const [shareStatus, setShareStatus] = useState<'idle' | 'copied' | 'error'>('idle');
  const [filterCategory, setFilterCategory] = useState<string>(t('results.all'));
  ...
  const handleDownloadPDF = async () => {
    ...
    const opt = {
      margin: 0,
      filename: `${t('app.name')}_Report_${profile.name.replace(/\s+/g, '_')}.pdf`,
  ...
  const handleShare = async () => {
    const shareData = {
      title: t('results.shareTitle', { appName: t('app.name') }),
      text: t('results.shareText', { draftingFor: t('quiz.draftingFor', { name: profile.name }), appName: t('app.name') }),
  ...
  const categoryMeta = useMemo(() => {
    ...
    return {
      all: [t('results.all'), ...cats.map(k => categoryNameMap[k])],
      keys: cats,
  ...
  const filteredAnswers = useMemo(() => {
    if (filterCategory === t('results.all')) return answers;
    return answers.filter(ans => {
      const q = questions.find(q => q.id === ans.questionId);
      return q?.category[language] === filterCategory;
    });
  }, [answers, filterCategory, questions, language]);
  ...
  const CorrectIcon = ({ className = "w-4 h-4" }: { className?: string }) => (
    <svg aria-label={t('results.correctAriaLabel')} role="img" className={`${className} text-emerald-700`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path>
    </svg>
  );

  const IncorrectIcon = ({ className = "w-4 h-4" }: { className?: string }) => (
    <svg aria-label={t('results.incorrectAriaLabel')} role="img" className={`${className} text-rose-700`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000 relative">
      <section className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 no-print" aria-labelledby="customization-heading">
        <div className="flex items-center justify-between mb-4">
          <div className="flex flex-col">
            <h3 id="customization-heading" className="font-bold text-slate-800 flex items-center gap-2">
              ...
              {t('results.reportInsights')}
            </h3>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">{t('results.customSync')}</p>
          </div>
          <div className="flex items-center gap-3">
            {isSaving && (
              <div className="flex items-center gap-2 animate-pulse">
                ...
                <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{t('results.saving')}</span>
              </div>
            )}
            {saveFeedback && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button onClick={() => toggleSetting('showChart')} aria-pressed={settings.showChart} className={`flex items-center justify-between p-3 rounded-xl border-2 transition-all ${settings.showChart ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-100 bg-slate-50 text-slate-500'}`}>
            <span className="text-sm font-bold">{t('results.competencyTitle')}</span>
          ...
          {/* <button onClick={() => toggleSetting('showDetailed')} aria-pressed={settings.showDetailed} className={`flex items-center justify-between p-3 rounded-xl border-2 transition-all ${settings.showDetailed ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-100 bg-slate-50 text-slate-500'}`}>
            <span className="text-sm font-bold">{t('results.detailedTitle')}</span>
      ...
      <div ref={reportRef} className="space-y-8">
        <header className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 p-8 md:p-12 border border-slate-100 overflow-hidden relative">
          ...
          <div className="relative flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div className="space-y-4">
              <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight leading-none">{t('results.summaryTitle')}</h1>
            ...
            <div className="flex flex-col items-center md:items-end">
              <div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-2">{t('results.overallScore')}</div>
              <div className="flex items-baseline gap-1">
                <span className="text-6xl font-black text-blue-600 leading-none" aria-label={`${t('results.scoreLabel')}: ${totalScore} / ${questions.length}`}>{totalScore}</span>
        ...
        {!isAuthenticated && (
          <section className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl shadow-xl p-8 md:p-10 text-white">
            <div className="flex flex-col md:flex-row items-center gap-8">
              {/* Minimalist Score Ring */}
              <div className="flex-shrink-0 relative">
                <svg className="w-40 h-40 transform -rotate-90" viewBox="0 0 120 120">
                  <circle cx="60" cy="60" r="52" fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="10" />
                  <circle
                    cx="60" cy="60" r="52" fill="none" stroke="white" strokeWidth="10"
                    strokeLinecap="round"
                    strokeDasharray={`${(totalScore / questions.length) * 326.7} 326.7`}
                    className="transition-all duration-1000 ease-out"
                  />
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-3xl font-black">{Math.round((totalScore / questions.length) * 100)}</span>
                  <span className="text-[10px] font-black uppercase tracking-widest opacity-70">{t('results.scoreLabel')}</span>
                </div>
              </div>

              {/* Minimal Parameters */}
              <div className="flex-1 w-full">
                <h2 className="text-xl font-black mb-4">{t('results.unlockFullReport')}</h2>
                <div className="grid grid-cols-3 gap-3 mb-6">
                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 text-center">
                    <div className="text-2xl font-black">{totalScore}</div>
                    <div className="text-[10px] font-bold uppercase tracking-wider opacity-70">{t('results.correctCount')}</div>
                  </div>
                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 text-center">
                    <div className="text-2xl font-black">{questions.length}</div>
                    <div className="text-[10px] font-bold uppercase tracking-wider opacity-70">{t('results.totalCount')}</div>
                  </div>
                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 text-center">
                    <div className="text-2xl font-black">{questions.length - totalScore}</div>
                    <div className="text-[10px] font-bold uppercase tracking-wider opacity-70">{t('results.wrongCount')}</div>
                  </div>
                </div>
                <button
                  onClick={onRegisterRequired}
                  className="inline-flex items-center gap-2 px-6 py-3 bg-white text-blue-600 rounded-xl font-bold text-sm hover:bg-blue-50 transition-colors"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clipRule="evenodd" />
                  </svg>
                  {t('results.createAccountToView')}
                </button>
              </div>
            </div>
          </section>
        ...
        {settings.showChart && isAuthenticated && (
            ...
            <div className="h-[350px] w-full" aria-hidden="true">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="80%" data={categoryMeta.chartData}>
                  ...
                  <Radar name={t('results.proficiency')} dataKey="score" stroke="#4f46e5" fill="#4f46e5" fillOpacity={0.15} strokeWidth={3} />
                </RadarChart>
              </ResponsiveContainer>
            </div>
          </section>
            ...
            <div className="flex flex-col space-y-6 mb-12">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <div className="flex items-center gap-4 no-print">
                  ...
                  <button onClick={onRestart} className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-sm font-bold rounded-xl transition-all shadow-xl shadow-slate-200 active:scale-95">{t('results.retake')}</button>
                </div>
              </div>
              ...
              <div className="no-print relative" role="tablist" aria-label="Filter">
                <div ref={tabsRef} className="flex items-center gap-1 overflow-x-auto no-scrollbar pb-4 scroll-smooth mask-linear-fade">
                  {categoryMeta.all.map(cat => {
                    const count = cat === t('results.all') ? answers.length : answers.filter(a => questions.find(q => q.id === a.questionId)?.category[language] === cat).length;
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pb-16 no-print">
        {/* <Tooltip text={t('results.isoA4Desc')}>
          <button
          ...
          >
            ...
            <div className="flex flex-col items-start">
              <span className="text-xs font-bold uppercase tracking-wider opacity-80 text-blue-100/60">{t('results.pdfLabel')}</span>
              <span className="font-bold text-sm">{t('results.exportPdf')}</span>
            </div>
          </button>
        </Tooltip>
        ...
        <Tooltip text={t('results.printReport')}>
          <button
          ...
          >
            ...
            <span className="font-bold text-sm">{t('results.printReport')}</span>
          </button>
        </Tooltip>

        <Tooltip text={shareStatus === 'copied' ? t('results.copied') : t('results.shareResults')}>
          <button
            onClick={handleShare}
            className="group flex items-center gap-3 px-6 py-4 w-full bg-white text-slate-700 rounded-2xl border border-slate-200 hover:border-blue-200 hover:bg-blue-50/50 hover:text-blue-700 transition-all shadow-sm hover:shadow-md active:scale-95"
          >
            ...
            <span className="font-bold text-sm">{shareStatus === 'copied' ? t('results.copied') : t('results.shareResults')}</span>
          </button>
        </Tooltip>
      </div>

      {/* Post-quiz actions */}
      <section className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 p-8 md:p-12 border border-slate-100 mb-16 no-print">
        <div className="text-center max-w-2xl mx-auto">
          <h2 className="text-2xl font-black text-slate-900 mb-3">
            {isAuthenticated ? t('results.readyForMore') : t('results.whatsNext')}
          </h2>
          <p className="text-slate-500 mb-8">
            {isAuthenticated ? t('results.readyForMoreDesc') : t('results.whatsNextDesc')}
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button
              onClick={onRestart}
              className="px-8 py-4 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl transition-all shadow-xl shadow-slate-200 active:scale-95 flex items-center gap-2"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            {!isAuthenticated && onRegisterRequired && (
              <button
                onClick={onRegisterRequired}
                className="px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all shadow-lg shadow-blue-200 active:scale-95 flex items-center gap-2"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                  <path d="M8 9a3 3 0 100-6 3 3 0 000 6zM8 11a6 6 0 016 6H2a6 6 0 016-6z" />
                </svg>
                {t('results.registerToSave')}
              </button>
            )}
          </div>
          {!isAuthenticated && onRegisterRequired && (
            <button
              onClick={onRegisterRequired}
              className="mt-4 text-slate-400 hover:text-blue-600 text-sm font-semibold transition-colors"
            >
              {t('results.alreadyHaveAccount')} <span className="underline">{t('results.loginNow')}</span>
            </button>
          )}
        </div>
      </section>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 animate-in fade-in duration-300" role="dialog" aria-modal="true" aria-labelledby="preview-modal-title">
          ...
          <div className="relative w-full max-w-7xl h-full max-h-[92vh] bg-slate-900 rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-slate-700">
              ...
              <div className="absolute inset-0 z-[110] bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center animate-in fade-in duration-300" role="status">
                {isGeneratingPDF ? (
                  <>
                    ...
                    <div className="mt-8 text-center"><h4 className="text-2xl font-black text-slate-900 tracking-tight">{t('results.finalizing')}</h4><p className="text-sm text-slate-500 mt-2 font-medium">{t('results.isoA4Desc')}</p></div>
                  </>
                ) : (
                  <div className="flex flex-col items-center animate-in zoom-in duration-500">
                    ...
                    <h4 className="text-3xl font-black text-slate-900 tracking-tight mb-2">{t('results.successTitle')}</h4>
                    <p className="text-slate-500 font-medium max-w-sm text-center px-4">{t('results.successDesc')}</p>
                    <button
            ...
            <div className="bg-slate-950 px-10 py-6 flex items-center justify-between border-b border-slate-800">
              <div className="flex items-center gap-5"><div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-lg" aria-hidden="true">Q</div><div><h3 id="preview-modal-title" className="text-xl font-black text-white tracking-tight leading-tight">{t('results.previewTitle')}</h3><p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">{t('results.isoA4')}</p></div></div>
              <button onClick={() => setShowPreview(false)} disabled={isGeneratingPDF || showSuccess} aria-label={t('common.close')} className="p-3 text-slate-400 hover:text-white hover:bg-slate-800 rounded-2xl transition-all"><svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
              ...
              <div className="relative w-full max-w-[210mm] min-h-[297mm] bg-white shadow-[0_0_100px_rgba(0,0,0,0.6)] overflow-hidden origin-top my-6">
                <div ref={printTemplateRef} className="w-full bg-white min-h-[297mm] p-[25mm_30mm] flex flex-col text-slate-900">
                  <div className="border-b-[6px] border-blue-600 pb-10 flex justify-between items-start mb-14">
                    <div className="space-y-6"><div className="flex items-center gap-4"><div className="w-12 h-12 bg-slate-950 rounded-xl flex items-center justify-center text-white font-black text-2xl shadow-xl shadow-blue-200">Q</div><div className="flex flex-col"><span className="text-2xl font-black text-slate-950 tracking-tighter uppercase leading-none">{t('app.name')}</span><span className="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em] mt-1">{t('results.intelligence')}</span></div></div><div><h1 className="text-5xl font-black text-slate-950 tracking-tighter leading-none mb-2">{t('results.professionalAssessment')}</h1><div className="flex items-center gap-4 text-slate-600 text-[11px] font-black uppercase tracking-widest"><span className="text-blue-600">{profile.name}</span><span className="opacity-20">|</span><span>{profile.role}</span></div></div></div>
                    <div className="text-right flex flex-col items-end pt-2"><div className="px-8 py-5 bg-slate-950 text-white rounded-2xl mb-3 shadow-2xl shadow-slate-300"><span className="text-[10px] block font-black uppercase opacity-60 tracking-[0.1em] mb-1">{t('results.finalIndex')}</span><span className="text-4xl font-black block leading-none">{totalScore} / {questions.length}</span></div><span className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">{t('results.verifiedResult')}</span></div></div>
                  <div className="flex-grow flex flex-col gap-16">

                    {settings.showChart && (<section className="space-y-6"><div className="flex items-center gap-3 border-l-[6px] border-blue-600 pl-6 py-2"><h2 className="text-[12px] font-black text-slate-950 uppercase tracking-[0.25em]">{t('results.competencyArch')}</h2></div><div className="h-[360px] w-full border border-slate-100 rounded-[2.5rem] p-12 bg-white shadow-sm flex items-center justify-center"><ResponsiveContainer width="100%" height="100%"><RadarChart cx="50%" cy="50%" outerRadius="80%" data={categoryMeta.chartData}><PolarGrid stroke="#e2e8f0" strokeWidth={1} /><PolarAngleAxis dataKey="category" tick={{ fontSize: 11, fontWeight: 'bold', fill: '#1e293b' }} /><Radar name={t('results.proficiency')} dataKey="score" stroke="#4f46e5" fill="#4f46e5" fillOpacity={0.15} strokeWidth={3} /></RadarChart></ResponsiveContainer></div></section>)}
                    {settings.showDetailed && (<section className="space-y-10 pb-16"><div className="flex items-center gap-3 border-l-[6px] border-blue-600 pl-6 py-2"><h2 className="text-[12px] font-black text-slate-950 uppercase tracking-[0.25em]">{t('results.performanceMatrix')}</h2></div><div className="grid grid-cols-1 gap-y-14">
                      {answers.map((ans, i) => {
                        const q = questions.find(q => q.id === ans.questionId)!;
                        const isCorrect = ans.answerIndex === q.correctAnswerIndex;
                  ...
                  <div className="mt-16 pt-10 border-t-2 border-slate-100 flex justify-between items-end text-[10px] text-slate-400 font-black uppercase tracking-[0.2em]"><div className="flex flex-col gap-2"><div className="flex items-center gap-5"><span className="text-slate-950">{t('app.name')} Insights &copy; {new Date().getFullYear()}</span><span className="opacity-20">•</span><span>{t('results.officialReport')}</span></div><div className="text-[9px] opacity-60 tracking-normal font-bold">{t('results.docId')}: <span className="text-slate-900">{docRef}</span></div></div><div className="text-right flex flex-col items-end gap-2"><div className="bg-slate-50 px-4 py-1.5 rounded-lg text-slate-950 border border-slate-200 font-black tracking-widest flex items-center gap-2"><span className="text-blue-600 opacity-60">{t('results.page')}</span> 01 / 01</div><div className="text-[9px] opacity-60 tracking-normal font-bold uppercase">{t('results.issued')}: {new Date().toLocaleDateString(language === 'id' ? 'id-ID' : 'en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</div></div></div>
                </div>
              </div>
            </div>
